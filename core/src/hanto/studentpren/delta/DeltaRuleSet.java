 ...
 * @version Feb 3, 2013
 */
public class DeltaRuleSet extends HantoRuleSet{
	 ...
	 */
	public DeltaRuleSet(HantoState gameState){
		state = gameState;
		this.gameState = (HantoDeltaState) gameState;
	}
	...
	public void doMakeTheMove(HantoPieceType pieceType, Coordinate from, Coordinate to){
		...
		newPiece = new DeltaHantoPiece(to, gameState.getPlayerOnMove(), pieceType, moveStrategy);
		if(from != null){
	...
	private boolean isOnlySamePieceAdjacentTo(Coordinate coordinate){
		...
		for(Coordinate c : neighbors){
			DeltaHantoPiece piece = (DeltaHantoPiece) gameState.getBoard().getPieceAt(c);
	...
	private void checkPieceValidMove(HantoPieceType pieceType, Coordinate from, Coordinate to) 
			throws HantoException{
		final DeltaHantoPiece piece = (DeltaHantoPiece) gameState.getBoard().getPieceAt(from);
