...
import hanto.studentpren.common.HantoRuleSet;
import hanto.studentpren.common.HantoState;
 ...
 * @version Feb 3, 2013
 */
public class GammaRuleSet extends HantoRuleSet{

	private final HantoGammaState gameState;
	 ...
	 */
	public GammaRuleSet(HantoState gameState){
		state = gameState;
		this.gameState = (HantoGammaState) gameState;
	}
	...
	public void doMakeTheMove(HantoPieceType pieceType, Coordinate from, Coordinate to){
		if(from == null){
			state.getBoard().putPieceAt(
				new GammaHantoPiece(to, gameState.getPlayerOnMove(), pieceType), to);
	...
	public MoveResult determineMoveResult(){
		...
		if(state.getTurnNum() > ((HantoGammaState) state).getMAX_TURNS()){
	...
	protected void checkMoveIsLegal(HantoPieceType pieceType, final Coordinate from,
		...
		if (pieceType == HantoPieceType.BUTTERFLY && from != null) {
			final GammaHantoPiece piece = (GammaHantoPiece) gameState.getBoard().getPieceAt(from);
